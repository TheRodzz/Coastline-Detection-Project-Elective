...
def select_seeds_using_svd(image, num_seeds):
    """
    Select seeds using Singular Value Decomposition (SVD)
    :param image: a 2D array representing the image
    :param num_seeds: the number of seeds to select
    :return: list of coordinates [row, column] for seeds
    """
    u, s, vh = np.linalg.svd(image)
    seeds = []
    for i in range(num_seeds):
        max_value = np.max(u[:, i])
        max_index = np.argmax(u[:, i])
        row = max_index // image.shape[1]
        col = max_index % image.shape[1]
...
if __name__ == '__main__':
    ...
    while(True):
        ...
        num_seeds = 3
        seeds = select_seeds_using_svd(sar_img, num_seeds)
        ...
        plt.tight_layout()
        # plt.show()
        plt.savefig(f"./results/SVDbased/cropped_image/ws={i}_non_speckle_filtered")
        plt.close()
